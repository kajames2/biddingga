#ifndef AUCTIONS_COMMON_VALUE_SIGNAL_SECOND_H_
#define AUCTIONS_COMMON_VALUE_SIGNAL_SECOND_H_
...
class CommonValueSignalSecond {
 public:
  CommonValueSignalSecond(std::vector<int> n_draws, float epsilon,
  ...
  std::vector<float> GetFitness(
      const std::vector<numericaldists::Scatter>& rel_bid_funcs, int id) const {
    if (!pre_calculated_) {
      Precalculate();
    }
...
#endif  // AUCTIONS_COMMON_VALUE_SIGNAL_SECOND_H_
