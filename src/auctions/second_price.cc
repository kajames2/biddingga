...
void SecondPrice::AcceptStrategy(std::function<float(float)> bid_func, int id) {
  bid_funcs_[id] = bid_func;
  pre_calculated_ = false;
}

float SecondPrice::GetFitness(const std::function<float(float)>& bid_func,
                              int id) const {
  if (!pre_calculated_) {
    Precalculate();
  }
...
void SecondPrice::Precalculate() const {
  ...
  std::vector<PiecewiseLinear> cdfs;
...
double SecondPrice::GetIntegrand(const std::function<float(float)>& bid_func,
