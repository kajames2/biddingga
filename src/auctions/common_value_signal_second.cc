...
CommonValueSignalSecond::CommonValueSignalSecond(std::vector<int> n_draws,
                                                 float epsilon,
    ...
    : n_players_(n_draws.size()),
      ...
      other_highest_cdfs_(n_draws.size()),
      exp_value_funcs_(n_draws.size()),
...
void CommonValueSignalSecond::AcceptStrategy(Scatter rel_bid_func, int id) {
  rel_bid_funcs_[id] = std::move(rel_bid_func);
  pre_calculated_ = false;
}

void CommonValueSignalSecond::AcceptStrategy(float rel_bid_value, int id) {
...
float CommonValueSignalSecond::GetFitness(const Scatter& rel_bid_func,
                                          int id) const {
  if (!pre_calculated_) {
    Precalculate();
  }
  ...
  ArrayXXd exp_second_bid_given_win(rel_bids_mesh.rows(), rel_bids_mesh.cols());
  for (int i = 0; i < rel_bids_mesh.rows(); ++i) {
    win_probs.row(i) =
        Interpolate(internal_bids_, other_highest_cdfs_[id], bids.row(i));
    exp_second_bid_given_win.row(i) = Interpolate(internal_bids_, exp_value_funcs_[id], bids.row(i));
  }
  // True value is the reference point, so it is 0.
  ArrayXXd profits = 0 - exp_second_bid_given_win;
  ArrayXXd utils =
...
float CommonValueSignalSecond::GetFitness(const float rel_bid, int id) const {
  if (!pre_calculated_) {
    Precalculate();
  }
  ...
  ArrayXd profits = 0 - exp_second_bid_given_win;
  ArrayXd utils =
      profits.unaryExpr(utility_funcs_[id]) *
          win_probs.unaryExpr(prob_weight_funcs_[id]) +
      utility_funcs_[id](0) * (1 - win_probs).unaryExpr(prob_weight_funcs_[id]);
...
void CommonValueSignalSecond::Precalculate() const {
