...
struct BinaryToGridMulti {
  GridMulti operator()(const Gen& gene) const {
    ...
    ArrayXXf xbids(1, size);
    ArrayXXf ybids(1, size);
    xbids.row(0) = ys.segment(0, size);
    ybids.row(0) = ys.segment(size, size);
    xbids.resize(valys_.size(), valxs_.size());
    ybids.resize(valys_.size(), valxs_.size());
      ...
      for (int col = 0; col < nx_chunks_; ++col) {
        ...
        zs.block(j, i, ys_per_chunk_ - 2, xs_per_chunk_ - 2) =
            strats[chunk_ind].phenotype.z_sets[zid].block(
                1, 1, ys_per_chunk_ - 2, xs_per_chunk_ - 2);
      }
    }
    ...
    for (int row = 1; row < ny_chunks_; ++row) {
      for (int col = 1; col < nx_chunks_; ++col) {
        ...
        zs(j, i) = (strats[tr_ind].phenotype.z_sets[zid](0, 0) +
                    ...
                    strats[bl_ind].phenotype.z_sets[zid](ys_per_chunk_ - 1,
                                                         xs_per_chunk_ - 1)) /
                   4;
      }
    }
