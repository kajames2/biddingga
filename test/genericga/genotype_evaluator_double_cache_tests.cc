...
class GenotypeEvaluatorDoubleCacheTest : public ::testing::Test {
 public:
  GenotypeEvaluatorDoubleCacheTest()
      : gene_eval(SamplePhenotypeConverter(100), SampleFitnessCalculator(5)) {}

 protected:
  virtual void SetUp() {}
  genericga::GenotypeEvaluatorDoubleCache<int, int> gene_eval;
};

TEST_F(GenotypeEvaluatorDoubleCacheTest, GetFitnessTest) {
  EXPECT_EQ(24, gene_eval.GetFitness(9));
}

TEST_F(GenotypeEvaluatorDoubleCacheTest, SetFitnessUpdatesCacheTest) {
  gene_eval.GetFitness(9);
  gene_eval.SetFitnessCalculator(SampleFitnessCalculator(6));
  EXPECT_EQ(25, gene_eval.GetFitness(9));
}

TEST_F(GenotypeEvaluatorDoubleCacheTest, GetFitnessesTest) {
  std::vector<float> exp_fits{24, 80, 69};
  EXPECT_EQ(exp_fits, gene_eval.GetFitnesses(std::vector<int>{9, 5, 6}));
}

TEST_F(GenotypeEvaluatorDoubleCacheTest, GetPhenotypeStrategyTest) {
...
TEST_F(GenotypeEvaluatorDoubleCacheTest, GetPhenotypeStrategiesTest) {
